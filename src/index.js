...
const TURN_SEC = 30;
const IDLE_SEC = 60;
const COL_COLOR = { A:"#e53935", B:"#1e88e5", C:"#e53935", D:"#1e88e5" };
const COL_BG    = { A:"#c62828", B:"#1565c0", C:"#c62828", D:"#1565c0" };
...
/* ══════════════════════════════════════════
   FIELD BUTTON — large, rounded, colorful
══════════════════════════════════════════ */
function FieldBtn({field, state, canOpen, colId, onOpen}){
  const cc = COL_COLOR[colId];
  const bg = COL_BG[colId];

  const base = {
    width:"100%", height:52, borderRadius:12,
    display:"flex", alignItems:"center", justifyContent:"center",
    flexDirection:"column", gap:1, padding:"0 10px",
    boxSizing:"border-box", userSelect:"none",
    transition:"all .15s", cursor: canOpen&&state==="hidden"?"pointer":"default",
    border:"none", fontFamily:"inherit",
  };

  if(state==="solved") return(
    <div style={{...base, background:bg, boxShadow:`0 3px 0 rgba(0,0,0,.3)`}}>
  ...
  if(state==="clue") return(
    <div style={{...base, background:bg, boxShadow:`0 3px 0 rgba(0,0,0,.3)`, border:`2px solid #fff`}}>
      <div style={{fontSize:9,color:"rgba(255,255,255,.8)",letterSpacing:.5}}>CLUE</div>
  ...
  return(
    <div
      ...
      style={{...base,
        background: canOpen ? "#3a4a6b" : "#2a3555",
        boxShadow: canOpen ? "0 3px 0 rgba(0,0,0,.4)" : "none",
        opacity: canOpen ? 1 : 0.7,
      }}
      onMouseEnter={e=>{ if(canOpen) e.currentTarget.style.background="#4a5a8b"; }}
      onMouseLeave={e=>{ if(canOpen) e.currentTarget.style.background="#3a4a6b"; }}
    >
      <div style={{fontSize:15,fontWeight:900,color: canOpen?"#a0b4d0":"#5a6a8a",letterSpacing:1}}>{field.id}</div>
    </div>
  );
}

/* ── THEME BUTTON (? or revealed) ── */
function ThemeBtn({colId, solved, theme, disabled, onGuess}){
  const [v,setV]=useState(""); const [err,setErr]=useState(false); const [editing,setEditing]=useState(false);
  const inpRef=useRef();
  const cc=COL_COLOR[colId]; const bg=COL_BG[colId];

  if(solved) return(
    <div style={{width:"100%",height:48,borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",
      background:bg,boxShadow:"0 3px 0 rgba(0,0,0,.3)"}}>
      <span style={{fontSize:13,fontWeight:900,color:"#fff",letterSpacing:1}}>✓ {theme}</span>
    </div>
  );

  if(!disabled&&editing) return(
    <div style={{width:"100%",height:48,borderRadius:12,display:"flex",alignItems:"center",gap:6,padding:"0 8px",
      background:"#fff",boxShadow:"0 3px 0 rgba(0,0,0,.3)",boxSizing:"border-box",animation:err?"shake .4s":"none"}}>
      <input ref={inpRef} value={v} onChange={e=>setV(e.target.value)}
        onKeyDown={e=>{ if(e.key==="Enter"){ if(!onGuess(v)){setErr(true);setTimeout(()=>setErr(false),600);}else{setV("");setEditing(false);} } if(e.key==="Escape") setEditing(false); }}
      ...
      <button onClick={()=>{ if(!onGuess(v)){setErr(true);setTimeout(()=>setErr(false),600);}else{setV("");setEditing(false);} }}
        style={{background:bg,border:"none",borderRadius:8,padding:"6px 12px",color:"#fff",fontWeight:900,cursor:"pointer",fontSize:12}}>OK</button>
      <button onClick={()=>setEditing(false)} style={{background:"#ddd",border:"none",borderRadius:8,padding:"6px 10px",color:"#666",fontWeight:700,cursor:"pointer",fontSize:12}}>✕</button>
    </div>
  );

  return(
    <div onClick={disabled?undefined:()=>setEditing(true)}
      style={{width:"100%",height:48,borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",
        background:disabled?"#2a3555":"rgba(255,255,255,.15)",
        boxShadow:disabled?"none":"0 3px 0 rgba(0,0,0,.3)",
        cursor:disabled?"default":"pointer",border:`2px dashed ${disabled?"#3a4a6b":cc}`,
        transition:"all .15s"}}
    ...
    >
      <span style={{fontSize:22,color:disabled?"#3a4a6b":cc,fontWeight:900}}>?</span>
    </div>
  );
}
...
function FinalBtn({solved, answer, disabled, onGuess}){
  ...
  if(solved) return(
    <div style={{...FB2, background:"#f59e0b", boxShadow:"0 4px 0 rgba(0,0,0,.4)"}}>
      <span style={{fontSize:16,fontWeight:900,color:"#fff",letterSpacing:1}}>✓ {answer}</span>
    </div>
  );

  if(!disabled&&editing) return(
    <div style={{...FB2,background:"#fff",animation:err?"shake .4s":"none",flexDirection:"row",gap:8,padding:"0 10px"}}>
      <input ref={inpRef} value={v} onChange={e=>setV(e.target.value)}
        onKeyDown={e=>{ if(e.key==="Enter"){ if(!onGuess(v)){setErr(true);setTimeout(()=>setErr(false),600);}else{setV("");setEditing(false);} } if(e.key==="Escape") setEditing(false); }}
      ...
      <button onClick={()=>{ if(!onGuess(v)){setErr(true);setTimeout(()=>setErr(false),600);}else{setV("");setEditing(false);} }}
        style={{background:"#f59e0b",border:"none",borderRadius:8,padding:"7px 14px",color:"#fff",fontWeight:900,cursor:"pointer",fontSize:13}}>OK</button>
    </div>
  );

  return(
    <div onClick={disabled?undefined:()=>setEditing(true)} style={{...FB2,
      background:disabled?"#2a3555":"rgba(255,255,255,.15)",
      border:`3px dashed ${disabled?"#3a4a6b":"#f59e0b"}`,
...
const FB2={width:"100%",height:58,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",
  flexDirection:"column",gap:2,boxShadow:"0 4px 0 rgba(0,0,0,.4)",transition:"all .15s",userSelect:"none",boxSizing:"border-box"};

function Spin({msg}){
  return(
    <div style={S.root}><style>{CSS}</style>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,background:"#1a2a4a"}}>
...
export default function App(){
  ...
  /* ─── LOBBY ─── */
  if(scr==="lobby") return(
    <div style={{...S.root,background:"#1a2a4a"}}>
      <style>{CSS}</style>
      {/* Header */}
      <div style={{background:"#0f1a30",padding:"14px 16px",borderBottom:"2px solid #f59e0b",textAlign:"center",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:4}}>
          ...
          <div>
            <div style={{fontFamily:"'Black Ops One',cursive",fontSize:8,color:"rgba(255,255,255,.5)",letterSpacing:5}}>PROJECT</div>
        ...
        <div style={{fontFamily:"'Black Ops One',cursive",fontSize:12,color:"#f59e0b",letterSpacing:4}}>ASSOCIATIONS</div>
      </div>

      <div style={{flex:1,overflowY:"auto",padding:"16px"}}>
        {/* Wallet */}
        <div style={{background:"rgba(255,255,255,.05)",borderRadius:12,padding:"12px",marginBottom:12,border:"1px solid rgba(255,255,255,.1)"}}>
        ...
        {/* Name */}
        <input value={nm} onChange={e=>setNm(e.target.value)} placeholder="Your name / username"
        ...
        <div style={{display:"flex",borderRadius:12,overflow:"hidden",border:"1px solid rgba(255,255,255,.15)",marginBottom:12}}>
          {[["create","🎮 CREATE ROOM"],["join","🚪 JOIN ROOM"]].map(([m,l])=>(
            <button key={m} onClick={()=>{setMode(m);setJerr("");}} style={{flex:1,padding:"11px 0",fontFamily:"inherit",fontSize:11,fontWeight:700,cursor:"pointer",background:mode===m?"#f59e0b":"transparent",color:mode===m?"#fff":"rgba(255,255,255,.5)",border:"none",letterSpacing:1}}>{l}</button>
          ))}
        </div>

        {mode==="create"&&(
          <button onClick={doCreate} disabled={!nm.trim()||!uid||!wallet} style={{width:"100%",padding:"14px 0",background:nm.trim()&&wallet?"linear-gradient(90deg,#f59e0b,#d97706)":"rgba(255,255,255,.1)",color:nm.trim()&&wallet?"#fff":"rgba(255,255,255,.3)",border:"none",borderRadius:12,fontFamily:"inherit",fontSize:14,fontWeight:900,cursor:nm.trim()&&wallet?"pointer":"default",letterSpacing:2,boxShadow:nm.trim()&&wallet?"0 4px 0 rgba(0,0,0,.3)":"none"}}>
            🎮 CREATE ROOM
          </button>
        )}
        {mode==="join"&&(
          <div>
            <div style={{fontSize:9,color:"rgba(255,255,255,.4)",letterSpacing:2,marginBottom:8}}>ROOM ID</div>
            ...
            <button onClick={doJoin} disabled={!nm.trim()||!uid||!wallet||jin.length<6} style={{width:"100%",padding:"14px 0",background:jin.length===6&&nm.trim()&&wallet?"linear-gradient(90deg,#22c55e,#16a34a)":"rgba(255,255,255,.1)",color:jin.length===6&&nm.trim()&&wallet?"#fff":"rgba(255,255,255,.3)",border:"none",borderRadius:12,fontFamily:"inherit",fontSize:14,fontWeight:900,cursor:jin.length===6&&nm.trim()&&wallet?"pointer":"default",letterSpacing:2,boxShadow:jin.length===6&&wallet?"0 4px 0 rgba(0,0,0,.3)":"none"}}>
              🚪 JOIN ROOM
            </button>
          </div>
        )}
        ...
        {/* Rules */}
        <div style={{marginTop:16,background:"rgba(255,255,255,.05)",borderRadius:12,padding:"12px",border:"1px solid rgba(255,255,255,.1)"}}>
          ...
          {[["30s/turn","Timer starts when both players join. Open 1 field per turn."],
            ...
            ["Time up","30s expire = 1 field auto-reveals, turn passes."],
    ...
    <div style={{...S.root,background:"#1a2a4a"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,padding:20}}>
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:"rgba(255,255,255,.3)",marginTop:10}}>Opponent enters this code under JOIN ROOM</div>
        </div>
    ...
    <div style={{...S.root,background:"#1a2a4a"}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
        <div style={{width:"100%",maxWidth:400,background:"#0f1a30",borderRadius:20,padding:"28px 20px",textAlign:"center",border:"2px solid rgba(255,255,255,.1)"}}>
          ...
          <div style={{fontFamily:"'Black Ops One',cursive",fontSize:26,letterSpacing:3,marginTop:8,color:winner===myRole?"#22c55e":"#ef4444"}}>{winner===myRole?"YOU WIN!":"YOU LOSE"}</div>
          ...
          <div style={{display:"flex",gap:14,justifyContent:"center",marginBottom:20}}>
            {["p1","p2"].map(r=>(
              <div key={r} style={{flex:1,padding:"14px",borderRadius:14,background:r===winner?"rgba(34,197,94,.1)":"rgba(255,255,255,.05)",border:`2px solid ${r===winner?"#22c55e":"rgba(255,255,255,.1)"}`}}>
                ...
                <div style={{fontSize:32,fontWeight:900,color:r===winner?"#22c55e":"#fff"}}>{scores[r]||0}</div>
          ...
          <button onClick={doReset} style={{width:"100%",padding:"14px",background:"linear-gradient(90deg,#8b5cf6,#7c3aed)",color:"#fff",border:"none",borderRadius:12,fontFamily:"inherit",fontSize:14,fontWeight:900,cursor:"pointer",letterSpacing:2,boxShadow:"0 4px 0 rgba(0,0,0,.3)"}}>
            NEW GAME
          </button>
        </div>
      </div>
  ...
  /* ════════════════════════════════════════════════════
     GAME SCREEN — vertical layout like reference app
      ...
      <div style={{background:"#0f1a30",padding:"8px 12px",borderBottom:"2px solid #f59e0b",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:36,height:36,borderRadius:"50%",background:"#e53935",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Black Ops One',cursive",fontSize:12,color:"#fff",flexShrink:0}}>{(myRole==="p1"?myNm:opNm).slice(0,2).toUpperCase()}</div>
            <div>
              <div style={{fontSize:10,color:"rgba(255,255,255,.5)"}}>{myRole==="p1"?myNm:opNm}</div>
              <div style={{fontSize:22,fontWeight:900,color:"#fff",lineHeight:1}}>{myRole==="p1"?scores.p1:scores.p2}</div>
            </div>
          </div>

          {/* TIMER CENTER */}
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,minWidth:70}}>
            <div style={{width:52,height:52,borderRadius:"50%",background:"#0f1a30",border:`3px solid ${!started?"#555":finalPhase?isMy?"#f59e0b":"#555":isMy?"#22c55e":"#555"}`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 0 12px ${isMy&&started?"#22c55e44":"transparent"}`}}>
              <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:!started?"#555":finalPhase?isMy?"#f59e0b":"#555":isMy?"#22c55e":"#555",fontWeight:900}}>
            ...
            <div style={{fontSize:8,color:isMy&&started?"#22c55e":"rgba(255,255,255,.3)",letterSpacing:1,fontWeight:700}}>
              {!started?"WAITING":finalPhase?"FINAL":isMy?"YOUR TURN":"WAIT"}
            </div>
          </div>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",gap:8,justifyContent:"flex-end"}}>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:10,color:"rgba(255,255,255,.5)"}}>{myRole==="p2"?myNm:opNm}</div>
              <div style={{fontSize:22,fontWeight:900,color:"#fff",lineHeight:1}}>{myRole==="p2"?scores.p2:scores.p1}</div>
            </div>
            <div style={{width:36,height:36,borderRadius:"50%",background:"#1e88e5",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Black Ops One',cursive",fontSize:12,color:"#fff",flexShrink:0}}>{(myRole==="p2"?myNm:opNm).slice(0,2).toUpperCase()}</div>
          </div>
        </div>

        {/* Hint bar */}
        <div style={{textAlign:"center",marginTop:4,fontSize:10,color:isMy&&started?"#f59e0b":"rgba(255,255,255,.3)",fontWeight:700}}>
          {!started?"⏳ Waiting for opponent...":finalPhase?(isMy?"🎯 Tap ? to guess themes or final answer":"⏳ Opponent guessing..."):isMy?(canOpen?"👆 TAP A FIELD TO REVEAL ITS CLUE":canGuess?"💡 TAP ? TO GUESS A THEME OR FINAL":"..."):"⏳ OPPONENT'S TURN"}
        </div>
      </div>

      {/* BOARD */}
      <div style={{flex:1,overflowY:"auto",padding:"8px 10px"}}>
        <div style={{maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column",gap:5}}>

          {/* AB FIELDS — A1 A2 A3 A4 | B1 B2 B3 B4 */}
          {/* Column labels */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
            <div style={{textAlign:"center",paddingBottom:2,borderBottom:`2px solid ${COL_COLOR.A}`}}>
              <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:COL_COLOR.A,letterSpacing:3}}>A</span>
            </div>
            <div style={{textAlign:"center",paddingBottom:2,borderBottom:`2px solid ${COL_COLOR.B}`}}>
              <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:COL_COLOR.B,letterSpacing:3}}>B</span>
            </div>
          </div>

          {/* A and B fields side by side */}
          {[0,1,2,3].map(i=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              <FieldBtn field={colA.fields[i]} state={fstate(colA.fields[i].id)} canOpen={canOpen} colId="A" onOpen={doOpen}/>
              <FieldBtn field={colB.fields[i]} state={fstate(colB.fields[i].id)} canOpen={canOpen} colId="B" onOpen={doOpen}/>
            </div>
          ))}
          ...
          {/* C and D fields side by side */}
          {[0,1,2,3].map(i=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              <FieldBtn field={colC.fields[i]} state={fstate(colC.fields[i].id)} canOpen={canOpen} colId="C" onOpen={doOpen}/>
              <FieldBtn field={colD.fields[i]} state={fstate(colD.fields[i].id)} canOpen={canOpen} colId="D" onOpen={doOpen}/>
            </div>
          ))}

          {/* Column labels CD */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
            <div style={{textAlign:"center",paddingTop:2,borderTop:`2px solid ${COL_COLOR.C}`}}>
              <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:COL_COLOR.C,letterSpacing:3}}>C</span>
            </div>
            <div style={{textAlign:"center",paddingTop:2,borderTop:`2px solid ${COL_COLOR.D}`}}>
              <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:COL_COLOR.D,letterSpacing:3}}>D</span>
            </div>
          </div>
          ...
          <div style={{display:"flex",gap:8,alignItems:"flex-start",marginTop:4}}>
            {isMy&&!finalPhase&&didOpen&&started&&(
              <button onClick={()=>{touch();doPass();L("Passed.");}}
...
const S={
  root:{height:"100vh",display:"flex",flexDirection:"column",background:"#1a2a4a",fontFamily:"'Rajdhani','Oswald',sans-serif",color:"#fff",overflow:"hidden"},
};
...
const CSS=`
  ...
  *{box-sizing:border-box}
  ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:rgba(0,0,0,.2)}::-webkit-scrollbar-thumb{background:rgba(255,255,255,.15);border-radius:2px}
`;
