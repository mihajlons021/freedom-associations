...
/* ═══════════════════════════════════════════════════════════
   CONSTANTS
═══════════════════════════════════════════════════════════ */
const ESCROW_ADDRESS = "GynyDkXj8WVdP7XDL1nTekF7Azv7ebxA7RCMnY3a3tSu";
const FREEDOM_TOKEN_MINT = "So11111111111111111111111111111111111111112"; // Replace with real FREEDOM mint
...
async function sendWager(amount, walletAddress) {
  try {
    ...
    if (!Connection) {
      // Fallback: just record the wager in Firebase without actual transfer
      console.log("Solana web3 not available, recording wager only");
      return { success: true, txid: "DEMO_TX_" + Date.now() };
    }

    const connection = new Connection(HELIUS_RPC);
    const fromPubkey = new PublicKey(walletAddress);
    const toPubkey = new PublicKey(ESCROW_ADDRESS);

    const transaction = new Transaction().add(
      SystemProgram.transfer({
        fromPubkey,
        toPubkey,
        lamports: amount * 1000, // token units
      })
    );
...
{
  "columns": [
    {"id":"A","theme":"THEME","fields":[{"id":"A1","clue":"short clue","answer":"ANSWER"},{"id":"A2","clue":"short clue","answer":"ANSWER"},{"id":"A3","clue":"short clue","answer":"ANSWER"},{"id":"A4","clue":"short clue","answer":"ANSWER"}]},
...
Rules: 4 completely different themes, short English clues 1-3 words, single-word CAPS answers, final answer is supercategory connecting all 4 themes.` }]
      })
    });
...
const Skull=({size=32})=>(
  <svg width={size} height={size} viewBox="0 0 64 64" fill="none">
    ...
    <polygon points="32,2 54,16 54,40 32,54 10,40 10,16" fill="none" stroke="#22c55e" strokeWidth="1" opacity=".3" transform="rotate(30,32,32)"/>
    <circle cx="32" cy="26" r="16" fill="#fff" opacity=".92"/>
    <rect x="24" y="38" width="7" height="9" rx="2" fill="#fff" opacity=".92"/>
    <rect x="33" y="38" width="7" height="9" rx="2" fill="#fff" opacity=".92"/>
    <ellipse cx="26" cy="24" rx="4.5" ry="5.5" fill="#111"/>
    <ellipse cx="38" cy="24" rx="4.5" ry="5.5" fill="#111"/>
    <ellipse cx="26" cy="23" rx="1.5" ry="2" fill="#8b5cf6" opacity=".8"/>
    <ellipse cx="38" cy="23" rx="1.5" ry="2" fill="#22c55e" opacity=".8"/>
    <path d="M28 34 L32 31 L36 34" stroke="#333" strokeWidth="1.5" fill="none"/>
    <line x1="27" y1="36" x2="27" y2="38" stroke="#333" strokeWidth="1"/>
    <line x1="32" y1="36" x2="32" y2="38" stroke="#333" strokeWidth="1"/>
    <line x1="37" y1="36" x2="37" y2="38" stroke="#333" strokeWidth="1"/>
  </svg>
);
...
function Header(){
  return(
    <div style={{
      width:"100%", background:"linear-gradient(90deg,#050505 60%,#0a0a0f)",
      borderBottom:"1px solid #1a1a2e",
      padding:"8px 16px", display:"flex", alignItems:"center",
      justifyContent:"space-between", boxSizing:"border-box", flexShrink:0,
      position:"relative", overflow:"hidden"
    }}>
      {/* Left green accent line */}
      <div style={{position:"absolute",left:0,top:0,bottom:0,width:3,background:"linear-gradient(180deg,#22c55e,#8b5cf6)"}}/>
      <div style={{display:"flex",alignItems:"center",gap:12,paddingLeft:8}}>
        <Skull size={38}/>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:30,height:1,background:"#22c55e",opacity:.5}}/>
            <div style={{fontFamily:"'Black Ops One',cursive",fontSize:9,color:"#aaa",letterSpacing:5,lineHeight:1}}>PROJECT</div>
            <div style={{width:30,height:1,background:"#22c55e",opacity:.5}}/>
          </div>
          <div style={{fontFamily:"'Black Ops One',cursive",fontSize:22,color:"#22c55e",letterSpacing:5,lineHeight:1.1,textShadow:"0 0 20px #22c55e88, 0 0 40px #22c55e44"}}>FREEDOM</div>
          <div style={{fontFamily:"'Black Ops One',cursive",fontSize:11,color:"#8b5cf6",letterSpacing:4,lineHeight:1,textShadow:"0 0 12px #8b5cf666"}}>ASSOCIATIONS</div>
        </div>
      </div>
...
function Footer(){
  return(
    <div style={{
      width:"100%", background:"#050505", borderTop:"1px solid #111",
      padding:"6px 16px", display:"flex", alignItems:"center",
      justifyContent:"center", gap:8, boxSizing:"border-box", flexShrink:0
    }}>
      <div style={{width:20,height:1,background:"#8b5cf6",opacity:.4}}/>
      ...
      <span style={{fontFamily:"'Black Ops One',cursive",fontSize:13,letterSpacing:2}}>
        <span style={{color:"#8b5cf6"}}>DEGEN</span><span style={{color:"#22c55e"}}>SAFE</span><span style={{color:"#555"}}>.FUN</span>
      </span>
      <div style={{width:20,height:1,background:"#22c55e",opacity:.4}}/>
    </div>
  );
}
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:6}}>
      <div style={{flex:1,height:4,background:"#111",borderRadius:2,overflow:"hidden"}}>
        <div style={{width:`${pct}%`,height:"100%",background:hot?"#ef4444":"#22c55e",transition:"width 1s linear, background .3s",borderRadius:2,boxShadow:`0 0 6px ${hot?"#ef4444":"#22c55e"}88`}}/>
      </div>
      <span style={{fontFamily:"monospace",fontSize:11,minWidth:24,color:hot?"#ef4444":"#666",fontWeight:hot?700:400}}>{secs}s</span>
    </div>
  );
}
...
function Field({field,revealed,active,oppActive,canClick,canGuess,onReveal,onGuess,accent}){
  ...
  const base={borderRadius:5,height:40,display:"flex",alignItems:"center",justifyContent:"center",
    position:"relative",overflow:"hidden",transition:"all .15s",userSelect:"none",boxSizing:"border-box",width:"100%"};
  ...
  if(oppActive) return(
    <div style={{...base,background:"#0a0a14",border:`1px solid #8b5cf633`,animation:"pulse 1.5s infinite"}}>
...
function GuessRow({label,solved,solvedText,disabled,onGuess,accent}){
  ...
  return(
    <div style={{...GR,animation:wrong?"shake .4s":"none",opacity:disabled?.25:1}}>
      <span style={{fontSize:8,color:"#444",whiteSpace:"nowrap",letterSpacing:.5}}>{label}</span>
...
export default function App(){
  ...
  async function createGame(){
    ...
    setLoadingMsg("Processing wager...");
  ...
  async function joinGame(){
    ...
    if(!walletAddr){alert("Please connect Phantom wallet first!");return;}
    setScreen("loading"); setLoadingMsg("Processing wager...");
    const tx=await sendWager(wager,walletAddr);
    if(!tx.success){alert("Wager failed: "+tx.error);setScreen("lobby");return;}
    ...
    addLog("Joined! You are Player 2. Waiting for P1 to move.");
  }
  ...
  async function guessField(fid,val){
    ...
    addLog(`❌ Wrong for field ${fid}`);return false;
  }
  ...
  async function guessCol(cid,val){
    ...
    if(match(val,col.theme)){
      ...
      addLog(`✅ Column ${cid} theme correct! +20pts`);return true;
    }
    addLog(`❌ Wrong theme for column ${cid}`);return false;
  }
    ...
    <div style={S.root}><style>{CSS}</style><Header/>
      <div style={{flex:1,overflowY:"auto",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"12px"}}>
        <div style={S.card}>
          ...
          {/* Wallet */}
          <div style={{marginBottom:16,padding:12,background:"#060606",borderRadius:8,border:"1px solid #1a1a1a"}}>
            ...
            {walletAddr?(
              ...
              <button onClick={handleConnectWallet} disabled={walletLoading} style={{
                width:"100%",padding:"8px 0",background:"linear-gradient(90deg,#8b5cf6,#7c3aed)",
                border:"none",borderRadius:6,color:"#fff",fontFamily:"inherit",fontSize:11,
                fontWeight:700,cursor:"pointer",letterSpacing:1,
                boxShadow:"0 0 16px #8b5cf644"
              }}>
          ...
          <div style={{marginTop:12}}>
            ...
            <div style={{display:"flex",gap:6}}>
              {WAGERS.map(w=><button key={w} onClick={()=>setWager(w)} style={{
                flex:1,padding:"8px 0",borderRadius:6,fontFamily:"inherit",fontSize:13,fontWeight:700,cursor:"pointer",
                background:wager===w?"#8b5cf6":"#0e0e0e",color:wager===w?"#fff":"#8b5cf6",
                border:`1px solid ${wager===w?"#8b5cf6":"#2a2a2a"}`,
                boxShadow:wager===w?"0 0 14px #8b5cf633":"none"
              }}>{w}</button>)}
            </div>
          </div>
          ...
          <div style={{marginTop:14,display:"flex",gap:0,borderRadius:7,overflow:"hidden",border:"1px solid #1a1a1a"}}>
            {[["create","🎮 CREATE ROOM"],["join","🚪 JOIN ROOM"]].map(([m,label])=>(
              <button key={m} onClick={()=>setLobbyMode(m)} style={{
                flex:1,padding:"9px 0",fontFamily:"inherit",fontSize:10,fontWeight:700,cursor:"pointer",
                background:lobbyMode===m?"#8b5cf6":"#0a0a0a",
                color:lobbyMode===m?"#fff":"#555",border:"none",letterSpacing:1,transition:"all .2s"
              }}>{label}</button>
            ))}
          </div>
          ...
          {/* CREATE */}
          {lobbyMode==="create"&&(
            <button onClick={createGame} disabled={!playerName.trim()||!uid||!walletAddr} style={{
              ...S.btn,marginTop:10,
              background:playerName.trim()&&walletAddr?"linear-gradient(90deg,#8b5cf6,#7c3aed)":"#1a1a1a",
              color:playerName.trim()&&walletAddr?"#fff":"#444",
              boxShadow:playerName.trim()&&walletAddr?"0 0 20px #8b5cf644":"none"
            }}>
          ...
          {lobbyMode==="join"&&(
            <div style={{marginTop:10}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <input value={joinId} onChange={e=>setJoinId(e.target.value.toUpperCase())} maxLength={6}
                  placeholder="ABC123" style={{
                    ...S.input,flex:1,textAlign:"center",letterSpacing:6,
                    fontFamily:"'Black Ops One',cursive",fontSize:18,color:"#22c55e",
                    boxShadow:joinId.length===6?"0 0 12px #22c55e33":"none"
                  }}/>
              </div>
              <button onClick={joinGame} disabled={!playerName.trim()||!uid||!walletAddr||joinId.length<6} style={{
                ...S.btn,marginTop:8,
                background:joinId.length===6&&playerName.trim()&&walletAddr?"linear-gradient(90deg,#22c55e,#16a34a)":"#1a1a1a",
                color:joinId.length===6&&playerName.trim()&&walletAddr?"#000":"#444",
                boxShadow:joinId.length===6&&walletAddr?"0 0 20px #22c55e44":"none"
              }}>
                🚪 JOIN ROOM
              </button>
            </div>
          ...
          <div style={{fontSize:8,color:"#1a1a1a",fontFamily:"monospace",textAlign:"center",marginTop:8}}>
            Escrow: {ESCROW_ADDRESS.slice(0,12)}…{ESCROW_ADDRESS.slice(-6)}
          </div>
  ...
  if(screen==="loading") return(
    <div style={S.root}><style>{CSS}</style><Header/>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
        ...
        <div style={{color:"#555",fontSize:12,letterSpacing:2}}>{loadingMsg}</div>
      </div>
    <Footer/></div>
  );
  ...
  if(screen==="waiting") return(
    <div style={S.root}><style>{CSS}</style><Header/>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,padding:20}}>
        ...
        <div style={{fontSize:11,color:"#444"}}>Wager: <b style={{color:"#a78bfa"}}>{wager}</b> FREEDOM tokens locked in escrow</div>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:10,color:"#555",marginBottom:8,letterSpacing:1}}>SHARE THIS ROOM ID WITH YOUR OPPONENT:</div>
          <div style={{
            fontSize:36,fontFamily:"'Black Ops One',cursive",color:"#22c55e",letterSpacing:8,
            padding:"14px 28px",background:"#0a1a0f",border:"2px solid #22c55e44",borderRadius:10,
            textShadow:"0 0 20px #22c55e88",boxShadow:"0 0 30px #22c55e22"
          }}>{gameId}</div>
          <div style={{fontSize:9,color:"#333",marginTop:8}}>OR opponent can auto-match with same wager ({wager} tokens)</div>
        </div>
        <button onClick={()=>{
          set(ref(db,`assoc_games/${gameId}`),null);
          setScreen("lobby");setGameId(null);setMyRole(null);
        }} style={{...S.btn,maxWidth:180,padding:"8px 0",fontSize:10}}>CANCEL</button>
      </div>
    <Footer/></div>
  );
    ...
    <div style={S.root}><style>{CSS}</style><Header/>
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
        <div style={{...S.card,textAlign:"center",maxWidth:340}}>
          <div style={{fontSize:56,lineHeight:1,filter:winner===myRole?"drop-shadow(0 0 20px gold)":""}}>{winner===myRole?"🏆":"💀"}</div>
          <div style={{fontFamily:"'Black Ops One',cursive",fontSize:24,letterSpacing:3,marginTop:8,
            color:winner===myRole?"#22c55e":"#ef4444",
            textShadow:`0 0 20px ${winner===myRole?"#22c55e":"#ef4444"}88`}}>
            {winner===myRole?"YOU WIN!":"YOU LOSE"}
          </div>
          <div style={{color:"#555",fontSize:11,marginTop:6,marginBottom:20}}>{winReason}</div>
          <div style={{display:"flex",gap:14,justifyContent:"center",marginBottom:20}}>
            {["p1","p2"].map(r=>(
              <div key={r} style={{padding:"12px 24px",borderRadius:10,
                background:r===winner?"#22c55e0a":"#0a0a0a",
                border:`2px solid ${r===winner?"#22c55e":"#1a1a1a"}`}}>
          ...
          {winner===myRole&&(
            <div style={{color:"#a78bfa",fontSize:12,marginBottom:14,padding:"8px",background:"#a78bfa0a",borderRadius:6,border:"1px solid #a78bfa33"}}>
              🎉 Winnings: <b>{wager*2}</b> FREEDOM tokens sent to your wallet
            </div>
          )}
  ...
  if(!board) return(
    <div style={S.root}><style>{CSS}</style><Header/>
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{color:"#444",fontSize:12}}>Loading game...</div>
      </div>
    <Footer/></div>
  );

  const [colA,colB,colC,colD]=[board.columns[0],board.columns[1],board.columns[2],board.columns[3]];
  ...
  return(
    <div style={S.root}><style>{CSS}</style><Header/>
        ...
        <div style={{display:"flex",gap:5,alignItems:"center",flex:1,minWidth:0}}>
          {["p1","p2"].map(r=>(
            <div key={r} style={{
              ...
              background:gameState?.currentTurn===r?"#8b5cf618":"#0a0a0a",
              border:`1px solid ${gameState?.currentTurn===r?"#8b5cf6":"#1a1a1a"}`,
              fontSize:10,color:r===myRole?"#a78bfa":"#555",
              fontWeight:gameState?.currentTurn===r?700:400,whiteSpace:"nowrap",
              boxShadow:gameState?.currentTurn===r?"0 0 10px #8b5cf622":""
            }}>
      ...
      {/* HINT */}
      <div style={{fontSize:9,textAlign:"center",padding:"3px 0",borderBottom:"1px solid #0a0a0a",
        color:isMy&&canClick?"#8b5cf6":isMy&&activeField?"#a78bfa":"#222",background:"#050505"}}>
        {finalPhase?"🎯 Guess column theme or final answer!"
          :isMy?canClick?"👆 Click any field to reveal it":activeField?`Field ${activeField} — type answer, theme, or final`:"Waiting for next turn..."
      ...
      <div style={{flex:1,overflowY:"auto",padding:"6px 4px"}}>
        <div style={{maxWidth:880,margin:"0 auto"}}>
          ...
          {/* ══ X-SHAPE BOARD ══
              Layout:
              [A header][A header][A header][A header]  [FINAL]  [D header][D header][D header][D header]
              [  A1  ][  A2  ][  A3  ][  A4  ]  [  ???  ]  [  D4  ][  D3  ][  D2  ][  D1  ]
              [  B1  ][  B2  ][  B3  ][  B4  ]  [       ]  [  C4  ][  C3  ][  C2  ][  C1  ]
              [B header][B header][B header][B header]  [FINAL]  [C header][C header][C header][C header]
          */}

          {/* Top column headers: A and D */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 80px 1fr",gap:4,marginBottom:3}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
              {/* A header */}
              <div style={{gridColumn:"1/3",textAlign:"center",padding:"4px 0",borderBottom:`2px solid ${ACCENTS.A}55`}}>
                <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:ACCENTS.A,letterSpacing:3,textShadow:`0 0 8px ${ACCENTS.A}66`}}>A</span>
              </div>
            </div>
            <div style={{textAlign:"center",padding:"4px 0",borderBottom:"1px solid #1a1a1a"}}>
              <span style={{fontSize:8,color:"#333",letterSpacing:1}}>FINAL</span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
              {/* D header */}
              <div style={{gridColumn:"1/3",textAlign:"center",padding:"4px 0",borderBottom:`2px solid ${ACCENTS.D}55`}}>
                <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:ACCENTS.D,letterSpacing:3,textShadow:`0 0 8px ${ACCENTS.D}66`}}>D</span>
              </div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 80px 1fr",gap:4,marginBottom:3}}>
            {/* A fields: A1 A2 A3 A4 */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:3}}>
              {colA.fields.map(f=>(
                <Field key={f.id} field={f} revealed={!!revealed[f.id]} active={activeField===f.id}
                  oppActive={oppAf===f.id} canClick={canClick} canGuess={canGuess&&activeField===f.id}
                  onReveal={revealField} onGuess={guessField} accent={ACCENTS.A}/>
              ))}
            </div>
            {/* FINAL CENTER — spans rows 1+2 */}
            <div style={{gridRow:"1/3",display:"flex",flexDirection:"column",alignItems:"stretch",justifyContent:"stretch"}}>
              <div style={{
                flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                background:finalPhase?"#a78bfa08":"#050508",
                border:`2px solid ${finalPhase?"#a78bfa55":"#111"}`,
                borderRadius:8,padding:8,textAlign:"center",
                animation:finalPhase?"glowPulse 2s infinite":"none",
                boxSizing:"border-box",minHeight:86
              }}>
                <div style={{fontSize:8,color:"#333",letterSpacing:1,marginBottom:3}}>FINAL ANSWER</div>
                {finalSolved
                ...
                <div style={{fontSize:7,color:"#2a2a2a",marginTop:3}}>{board.final.hint}</div>
              </div>
            </div>
            {/* D fields reversed: D4 D3 D2 D1 */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:3}}>
          ...
          {/* Row 2: B fields | (FINAL continued) | C fields (reversed) */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 80px 1fr",gap:4,marginBottom:3}}>
            ...
            {/* spacer — FINAL already rendered above with gridRow span */}
            <div/>
            {/* C fields reversed: C4 C3 C2 C1 */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:3}}>
          ...
          {/* Bottom column headers: B and C */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 80px 1fr",gap:4,marginBottom:4}}>
            <div style={{textAlign:"center",padding:"3px 0",borderTop:`2px solid ${ACCENTS.B}55`}}>
              <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:ACCENTS.B,letterSpacing:3,textShadow:`0 0 8px ${ACCENTS.B}66`}}>B</span>
            </div>
            <div/>
            <div style={{textAlign:"center",padding:"3px 0",borderTop:`2px solid ${ACCENTS.C}55`}}>
              <span style={{fontFamily:"'Black Ops One',cursive",fontSize:16,color:ACCENTS.C,letterSpacing:3,textShadow:`0 0 8px ${ACCENTS.C}66`}}>C</span>
            </div>
          </div>
